...
func findAuditWithOptions(s *discordgo.Session, guildID string, auditType int, ownerID string) (*discordgo.AuditLogEntry, interface{}, error) {
	...
	// Fast-path: direct user whitelist does not need member fetch/API calls.
	if database.Database.IsWhitelisted(guildID, "users", auditLog.UserID, nil) {
		return auditLog, nil, fmt.Errorf("Whitelisted")
	}

	// Only resolve full member when role-whitelist is configured.
	if !database.Database.HasWhitelistEntries(guildID, "whitelisted-roles") {
...
func SendAntiNukeLogToChannel(s *discordgo.Session, guildID, logChannel string, info AntiNukeLogData) {
	...
	victim := "N/A"
	if strings.TrimSpace(info.VictimName) != "" {
		victim = info.VictimName
	} else if info.VictimID != "" {
	...
	rightBlock := strings.Join([]string{
		fmt.Sprintf("**Action Taken:** %s", strings.ToUpper(info.CounterAction)),
		fmt.Sprintf("⏱**Response Time:** %s", info.ActionDuration),
		fmt.Sprintf("**Guild:** `%s`", guildID),
	}, "\n")

	embed := &discordgo.MessageEmbed{
		Title:       fmt.Sprintf("<:antiraid:1488849925822353448> PlayZ Security Alert • %s", info.EventTitle),
		Description: "High-risk action detected and handled automatically.",
		Color:       AntiNukeLogColor,
		Fields: []*discordgo.MessageEmbedField{
			{Name: "> <:Asb_Caution:1461768122653474959> Incident Snapshot", Value: leftBlock, Inline: true},
			{Name: "> <:Hired_Promoted:1461769381020307623> Enforcement Details", Value: rightBlock, Inline: true},
		},
...
func handleAuditAction(s *discordgo.Session, guildID, reason string, auditType int, recovery RecoveryFunc, options AuditActionOptions) {
	...
	actionDuration := FormatActionLatency(actionStarted, entry.ID)
	go func() {
		if recovery != nil {
			_ = recovery(entry)
		}
		...
		SendAntiNukeLogToChannel(s, guildID, logChannel, AntiNukeLogData{
			...
			VictimName:     resolveVictimNameFromAudit(s, guildID, auditType, entry),
...
func resolveVictimNameFromAudit(s *discordgo.Session, guildID string, auditType int, entry *discordgo.AuditLogEntry) string {
	if entry == nil {
		return "N/A"
	}
	...
	case 1:
		...
		return "Guild"
	case 20, 22, 23, 25, 28:
		if user, err := s.User(entry.TargetID); err == nil && user != nil {
			return fmt.Sprintf("%s#%s", user.Username, user.Discriminator)
		}
		return fmt.Sprintf("User `%s`", entry.TargetID)
	case 10, 11, 12, 50:
		if ch, err := s.State.Channel(entry.TargetID); err == nil && ch != nil {
			return fmt.Sprintf("#%s", ch.Name)
		}
		if ch, err := s.Channel(entry.TargetID); err == nil && ch != nil {
			return fmt.Sprintf("#%s", ch.Name)
		}
		return fmt.Sprintf("Channel `%s`", entry.TargetID)
	case 30, 31, 32:
		if role, err := s.State.Role(guildID, entry.TargetID); err == nil && role != nil {
			return fmt.Sprintf("@%s", role.Name)
		}
		return fmt.Sprintf("Role `%s`", entry.TargetID)
	case 60, 61, 62, 90, 91, 92:
		for _, change := range entry.Changes {
			if change.Key == nil || *change.Key != discordgo.AuditLogChangeKeyName {
				continue
			}
			if name, ok := change.NewValue.(string); ok && strings.TrimSpace(name) != "" {
				return name
			}
		...
		if entry.TargetID != "" {
			return fmt.Sprintf("Asset `%s`", entry.TargetID)
		}
		return "Emoji/Sticker"
	default:
		if entry.TargetID != "" {
			return fmt.Sprintf("ID `%s`", entry.TargetID)
		}
		return "N/A"
	}
}
