...
func (cmd *Commands) AntiInvite(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	if !canManageAntiNuke(s, m) {
		sendError(s, m.ChannelID, "Only guild owner OR admin with role above bot can manage anti-nuke settings.", m.Author.Username)
		return
	}
...
func (cmd *Commands) AntiNuke(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	...
	statusEmoji := antiStatusEmoji(enabled)
	title := "Security"
	description := "Successfully enabled security settings."
	nextHint := "To disable again use `>antinuke off`"
	if !enabled {
		description = "Successfully disabled security settings."
		nextHint = "To enable again use `>antinuke on`"
	}
	...
	_, _ = s.ChannelMessageSendEmbed(m.ChannelID, &discordgo.MessageEmbed{
		...
		Description: strings.Join([]string{
			description,
			"",
			fmt.Sprintf("Current Status : %s", statusEmoji),
			"",
			fmt.Sprintf("┃ %s", nextHint),
		}, "\n"),
		Color:  ThemeColor,
...
func (cmd *Commands) AntiNukeStatus(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	...
	masterEnabled := settingBool(data, "antinuke-enabled", false)
	moduleStatus := func(enabled bool) string {
		if !masterEnabled {
			return antiStatusEmoji(false)
		}
		return antiStatusEmoji(enabled)
	}

	moduleLines := []string{
		fmt.Sprintf("Anti Invite: %s", moduleStatus(settingOnOff(data, "anti-invite", false))),
		fmt.Sprintf("Anti Ban: %s", moduleStatus(settingBool(data, "anti-ban", true))),
		fmt.Sprintf("Anti Kick: %s", moduleStatus(settingBool(data, "anti-kick", true))),
		fmt.Sprintf("Anti Prune: %s", moduleStatus(settingBool(data, "anti-prune", true))),
		fmt.Sprintf("Anti Bot: %s", moduleStatus(settingBool(data, "anti-bot", true))),
		fmt.Sprintf("Anti Name Change: %s", moduleStatus(settingBool(data, "anti-name-change", true))),
		fmt.Sprintf("Anti Widget Spam: %s", moduleStatus(settingBool(data, "anti-widget-spam", true))),
		fmt.Sprintf("Anti Role Create: %s", moduleStatus(settingBool(data, "anti-role-create", true))),
		fmt.Sprintf("Anti Role Delete: %s", moduleStatus(settingBool(data, "anti-role-delete", true))),
		fmt.Sprintf("Anti Role Update: %s", moduleStatus(settingBool(data, "anti-role-update", true))),
		fmt.Sprintf("Anti Channel Create: %s", moduleStatus(settingBool(data, "anti-channel-create", true))),
		fmt.Sprintf("Anti Channel Delete: %s", moduleStatus(settingBool(data, "anti-channel-delete", true))),
		fmt.Sprintf("Anti Channel Update: %s", moduleStatus(settingBool(data, "anti-channel-update", true))),
		fmt.Sprintf("Anti Emoji Create: %s", moduleStatus(settingBool(data, "anti-emoji-create", true))),
		fmt.Sprintf("Anti Emoji Update: %s", moduleStatus(settingBool(data, "anti-emoji-update", true))),
		fmt.Sprintf("Anti Emoji Delete: %s", moduleStatus(settingBool(data, "anti-emoji-delete", true))),
		fmt.Sprintf("Anti Sticker Create: %s", moduleStatus(settingBool(data, "anti-sticker-create", true))),
		fmt.Sprintf("Anti Sticker Update: %s", moduleStatus(settingBool(data, "anti-sticker-update", true))),
		fmt.Sprintf("Anti Sticker Delete: %s", moduleStatus(settingBool(data, "anti-sticker-delete", true))),
		fmt.Sprintf("Anti Everyone Mention: %s", moduleStatus(settingBool(data, "anti-everyone-mention", true))),
		fmt.Sprintf("Anti Here Mention: %s", moduleStatus(settingBool(data, "anti-here-mention", true))),
		fmt.Sprintf("Anti Webhook: %s", moduleStatus(settingBool(data, "anti-webhook-create", true))),
		fmt.Sprintf("Anti Guild Update: %s", moduleStatus(settingBool(data, "anti-guild-update", true))),
		fmt.Sprintf("Anti Vanity Steal: %s", moduleStatus(settingBool(data, "anti-vanity-steal", true))),
		fmt.Sprintf("Anti Member Role: %s", moduleStatus(settingBool(data, "anti-member-role", true))),
	}

	_, _ = s.ChannelMessageSendEmbed(m.ChannelID, &discordgo.MessageEmbed{
		Title: "<:antiraid:1488849925822353448>  PlayZ Security",
		Description: strings.Join([]string{
			"Anti-nuke protection overview for this server.",
			fmt.Sprintf("Master Protection: %s", antiStatusEmoji(masterEnabled)),
			"",
			"**Modules**",
			strings.Join(moduleLines, "\n"),
		}, "\n"),
		Fields: []*discordgo.MessageEmbedField{
			{Name: "<:settings:1488969685994049706> Enforcement", Value: fmt.Sprintf("Punishment: `%s`\nThreshold: `%s`", strings.ToUpper(mType), threshold), Inline: true},
...
func (cmd *Commands) ClearWhitelists(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	...
	_, _ = s.ChannelMessageSendEmbed(m.ChannelID, &discordgo.MessageEmbed{
		Description: fmt.Sprintf("<:Tick:1470868695835476048> Successfully cleared `%d` whitelist entries for this server.", count),
		Color:       ThemeColor,
		Footer:      &discordgo.MessageEmbedFooter{Text: fmt.Sprintf("Requested by: %s", m.Author.Username)},
	})
}
...
func (cmd *Commands) LoggingChannel(s *discordgo.Session, message *discordgo.Message, ctx *Context) {
	if !canManageAntiNuke(s, message) {
		sendError(s, message.ChannelID, "Only guild owner OR admin with role above bot can manage anti-nuke settings.", message.Author.Username)
		return
	}
...
func (cmd *Commands) Prefix(s *discordgo.Session, message *discordgo.Message, ctx *Context) {
	...
	cmd.setPrefixCache(message.GuildID, newPrefix)
...
func (cmd *Commands) Whitelist(s *discordgo.Session, message *discordgo.Message, ctx *Context) {
	...
	_, _ = s.ChannelMessageSendEmbed(message.ChannelID, &discordgo.MessageEmbed{
		Description: fmt.Sprintf("<:Tick:1470868695835476048> Successfully whitelisted **%s**", target.Username),
		Color:       ThemeColor,
		Footer:      &discordgo.MessageEmbedFooter{Text: fmt.Sprintf("Requested by: %s", message.Author.Username)},
	})
}
...
func (cmd *Commands) Unwhitelist(s *discordgo.Session, message *discordgo.Message, ctx *Context) {
	...
	target := message.Mentions[0]
	_, _ = s.ChannelMessageSendEmbed(message.ChannelID, &discordgo.MessageEmbed{
		Description: fmt.Sprintf("<:AdobeExpressfile:1489310761036546232> Successfully removed whitelist for **%s**", target.Username),
		Color:       ThemeColor,
		Footer:      &discordgo.MessageEmbedFooter{Text: fmt.Sprintf("Requested by: %s", message.Author.Username)},
	})
}
...
func antiStatusEmoji(enabled bool) string {
	if enabled {
		return "<:evo_antinuke_disable:1489191890778849280><:antinuke_enable:1489191815214272684>"
	}
	return "<:evo_antinuke_disabled:1489193788101361855><:Prime_enable:1489193875829428347>"
}
...
func settingOnOff(data map[string]interface{}, key string, fallback bool) bool {
	raw, ok := data[key]
	if !ok {
		return fallback
	}
	...
	case string:
		return strings.EqualFold(typed, "on")
	case bool:
		return typed
	default:
		return fallback
