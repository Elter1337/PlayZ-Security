...
func (db *SQLiteDB) getCachedWhitelistLookup(guildID string) (map[string]map[string]struct{}, bool) {
	...
	return entry.Whitelists, true
}

func (db *SQLiteDB) IsSettingEnabled(guildID, key string) bool {
	if guildID == "" || key == "" {
		return false
	}
	...
	if ok && time.Now().Before(entry.ExpiresAt) {
		if enabled, ok := entry.Data[key].(bool); ok {
			db.Mu.RUnlock()
			return enabled
		}
		db.Mu.RUnlock()
	...
	// Fall back to full data fetch.
	data, err := db.FindData(guildID)
	if err != nil {
		return false
	}
	if enabled, ok := data[key].(bool); ok {
		return enabled
	}
	return false
}
...
func (db *SQLiteDB) CreateGuild(_ *discordgo.User, guild *discordgo.Guild) {
	...
	defaults := map[string]interface{}{
		...
		"anti-emoji-update":     true,
		"anti-emoji-delete":     true,
		"anti-sticker-create":   true,
		"anti-sticker-update":   true,
		"anti-sticker-delete":   true,
		"guild-id":              guild.ID,
...
func (db *SQLiteDB) HasWhitelistEntries(guildID, typeKey string) bool {
	if lookup, ok := db.getCachedWhitelistLookup(guildID); ok {
		return len(lookup[typeKey]) > 0
	}

	return len(db.getWhitelist(guildID, typeKey)) > 0
}
