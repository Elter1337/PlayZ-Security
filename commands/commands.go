...
func (cmds *Commands) Match(s *discordgo.Session, raw *discordgo.Message, context *Context) (*Command, []string) {
	...
	prefix, err := cmds.getPrefix(raw.GuildID)
	if err != nil {
...
func (cmds *Commands) getPrefix(guildID string) (string, error) {
	now := time.Now()

	cmds.PrefixMu.RLock()
	if entry, ok := cmds.PrefixCache[guildID]; ok && now.Before(entry.ExpiresAt) && entry.Prefix != "" {
		cmds.PrefixMu.RUnlock()
		return entry.Prefix, nil
	}
	cmds.PrefixMu.RUnlock()

	prefix, err := database.Database.GetPrefix(guildID)
	if err != nil {
		return "", err
	}
	...
	cmds.setPrefixCache(guildID, prefix)
	return prefix, nil
}

func (cmds *Commands) setPrefixCache(guildID, prefix string) {
	...
	cmds.PrefixMu.Lock()
	if cmds.PrefixCache == nil {
		cmds.PrefixCache = make(map[string]PrefixCacheEntry)
	}
	cmds.PrefixCache[guildID] = PrefixCacheEntry{
		Prefix:    prefix,
		ExpiresAt: time.Now().Add(ttl),
	}
	cmds.PrefixMu.Unlock()
}
