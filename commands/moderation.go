...
func (cmd *Commands) Ban(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	if !canRunWhitelistedOrOwner(s, m) {
		sendError(s, m.ChannelID, "Only whitelisted users or the guild owner can use this command.", m.Author.Username)
		return
	}
...
func (cmd *Commands) Unban(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	...
	targetID := normalizeUserID(ctx.Fields[0])
	if targetID == "" {
		sendError(s, m.ChannelID, "Provide a valid user ID or mention. Example: `unban 123456789012345678`", m.Author.Username)
		return
	}

	reason := moderationReason(ctx)
	if err := s.GuildBanDelete(m.GuildID, targetID); err != nil {
		sendError(s, m.ChannelID, fmt.Sprintf("Could not unban <@%s>. Make sure the user is banned.", targetID), m.Author.Username)
		return
	}

	sendModerationResult(s, m.ChannelID, "Unban", targetID, m.Author.Username, reason)
}

func (cmd *Commands) MassUnban(s *discordgo.Session, m *discordgo.Message, ctx *Context) {
	bans, err := fetchAllGuildBans(s, m.GuildID)
	if err != nil {
...
func fetchAllGuildBans(s *discordgo.Session, guildID string) ([]*discordgo.GuildBan, error) {
	allBans := []*discordgo.GuildBan{}
	var afterID string

	for {
		bans, err := s.GuildBans(guildID, 1000, "", afterID)
		if err != nil {
			return nil, err
		}
		if len(bans) == 0 {
		...
		allBans = append(allBans, bans...)
		if len(bans) < 1000 {
			break
		}

		afterID = bans[len(bans)-1].User.ID
	}
...
func runMassUnban(s *discordgo.Session, guildID string) (unbanned int, protected int, err error) {
	bans, err := fetchAllGuildBans(s, guildID)
	if err != nil {
		return 0, 0, err
	}
...
func sendModerationResult(s *discordgo.Session, channelID, action, targetID, moderator, reason string) {
	emoji := "<:Tick:1470868695835476048>"
...
func canRunWhitelistedOrOwner(s *discordgo.Session, m *discordgo.Message) bool {
	...
	return database.Database.IsWhitelisted(m.GuildID, "users", m.Author.ID, authorMember)
}
