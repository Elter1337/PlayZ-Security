...
func buildHelpEmbed(s *discordgo.Session, requester, prefix, category string) *discordgo.MessageEmbed {
	...
	case "anti":
		...
		embed.Description = strings.Join([]string{
			...
			fmt.Sprintf("`%sclearwhitelist`", prefix),
		}, "\n")
	...
	case "moderation":
		...
		embed.Description = strings.Join([]string{
			fmt.Sprintf("`%sban @user` `%skick @user` `%sunban <user_id>` `%smassunban`", prefix, prefix, prefix, prefix),
	...
	default:
		...
		embed.Fields = []*discordgo.MessageEmbedField{
			...
			{Name: "<:mod:1488967409640542269> Moderation", Value: "Ban, kick, unban, lock/unlock, slowmode, massunban", Inline: true},
